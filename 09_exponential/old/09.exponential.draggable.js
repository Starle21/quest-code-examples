...
function renderSquare() {
  ...
  switch (stateSquare) {
    ...
    case "IS_SELECTED": {
      ...
      if (data.mousedown) {
        console.log("mouse down! enable drag");
        stateSquare = "IS_DRAGGED";
    ...
    case "IS_DRAGGED": {
      if (data.mouseup) {
        ...
        document.removeEventListener("mousemove", drawSquareWithProperties);
        document.removeEventListener("mouseup", drawSquareWithProperties);
...
function calculateDelta() {
  const svgBox = domSvg.getBoundingClientRect();
  const deltaX = data.mouse.x - svgBox.x - data.xCoord;
  const deltaY = data.mouse.y - svgBox.y - data.yCoord;
  const newSqrX = data.mouse.x - data.xCoord;
  const newSqrY = data.mouse.y - data.yCoord;
  data.xCoord = newSqrX;
  data.yCoord = newSqrY;
  console.log("move to", newSqrX, newSqrY);
  updateSquare({ x: newSqrX, y: newSqrY, side: data.side });
}
