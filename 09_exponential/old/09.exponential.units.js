...
let domUnitsLabel = undefined;
let domUnitsDiv = undefined;
let domUnitsSelect = undefined;
let stateUnits = "IS_HIDDEN";
let currentUnit = "CM";
let units = ["Please select unit", "CM", "M"];
...
function renderUnits() {
  switch (stateUnits) {
    case "IS_HIDDEN": {
      if (data.squareSelected) {
        stateUnits = "CM";
        domUnitsSelect = createSelect(units);
        domUnitsLabel = createDiv("in units:");
        domUnitsLabel.classList.add("unit");
        domUnitsDiv = createDiv(currentUnit);
        domUnitsDiv.classList.add("unit");
        domCoordinates.append(domUnitsSelect, domUnitsLabel, domUnitsDiv);
        break;
      }
      break;
    }
    case "CM": {
      if (!data.squareSelected) {
        stateUnits = "IS_HIDDEN";
        domUnitsLabel.remove();
        domUnitsDiv.remove();
        break;
      }
      if (currentUnit === "M") {
...
function createSelect(options) {
  ...
  domSelect.name = "units";
  options.map((o) => {
    const option = document.createElement("option");
    option.textContent = o;
    option === "Please select unit" ? (option.value = "") : (option.value = o);
    o === currentUnit ? (option.selected = true) : "";
    domSelect.appendChild(option);
    domSelect.oninput = handleSelect;
  });
  return domSelect;
}
