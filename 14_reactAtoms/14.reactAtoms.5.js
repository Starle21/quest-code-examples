...
function render(Component, DOMRoot) {
  ...
  if (!wipVDOM) {
    ...
    createDOMNodes(wipVDOM, _currentRoot);
    // apply effects - changes with respect to current state (null)
    commitRoot(wipVDOM, _currentRoot);
  }
...
// CREATE ACCESSORS
// diff - if current null - only create
...
function commitEffects(finishedTree) {
  finishedTree.forEach((effect, index) => {
    ...
    if (effect.flag === "CREATE") {
      commitRoot(effect, effect.return.accessor);
      effect.flag = null;
    }
