...
...
...
Triggers and when it updates, 
 - setState, which calls render(), not batched
 - could call render on root - (plus set root.update = true) - it would recalculate the whole tree
    - update would be set only on root, childUpdates would not be marked from source fiber, bcs we don't have that when rendering from the top (in React not marked with lanes and childLanes), so every prop is new pointer, so all recalculate
 - changing keys - not implemented

 - dispatch() from useReducer
 - useSyncExternalStore
 - force render - with useReducer or setState

 - flushSync (before unstable_batchedUpdates())
 - setState in calculation conditionally - schedules synchronous render after commit? (does not yield to browser to paint)

Bail out
 - current fiber exists ( already mounted memoized frame for a function component, so we have props to compare )
 - new wip effect is of same type
 - props are pointer equal - nothing above changed state
 - no state update flag on the fiber - inside state didn't change
 - no consumed context changed
 --> clones child fiber if there is childUpdate and makes it the wip
 --> updates return of child fiber, returns null - does not go further below - bails from the whole subtree and goes to complete calculation phase

 - memo does shallow equal on props, so if values in those props are the same (pointers or primitive values), it bails out (memo, useMemo, useCallback)

Runtime order of calling functions and hooks, execution order
 - state hook - runs before calculation (eager) / runs in calculation in functional component, marks fiber with update
 - calculation phase - goDown and goUp, mark with flags
 - commit phase
    - commit whole tree - delete, create, update on the whole tree
    - useLayoutEffect
 - let browser paint
